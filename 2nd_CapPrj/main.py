...
from utils import frame_height, frame_width, start_button_pos, reset_button_pos, button_width, button_height, ConstVelocity, Wheels_dist
from pure_pursuit import calculate_omega, calculate_wheel_velocities, velocities_to_RPM, adaptive_lookahead
...
openCV.initialize_window(
    ...
    mouse_callback,
    (clicked_points, reset_button_pos, start_button_pos, button_width, button_height, frame_height, start_pressed, interpolated_waypoints)
)
...
while True:
    ...
    if ids is not None and start_pressed[0]:
        ...
        Adaptive_lookahead_use = adaptive_lookahead(w1, w2, omega)
        Adaptive_lookahead_use_real = Adaptive_lookahead_use * 0.00102
        ...
        if len(interpolated_waypoints) <= 10:
            ...
            if distance_current <= 15:
                ...
                omega = calculate_omega(signed_distance, ConstVelocity, Adaptive_lookahead_use_real)
    ...
    if len(clicked_points) > 1:
        ...
        if interpolated_waypoints:
            closest_point, interpolated_waypoints = find_closest_point(center_coordinate, interpolated_waypoints, Adaptive_lookahead_use)
            if closest_point:
